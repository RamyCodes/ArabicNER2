...
def train_dev_test_split(input_files, output_path, train_ratio):
    segments = list()
    ...
    datasets = np.split(segments, [int(train_ratio*n), int((train_ratio)*n)])
...
def main(args):
    ...
    if args.task == "train_dev_test_split":
        train_dev_test_split(args.input_files, args.output_path, args.train_ratio)
